...
void LinesExtractor::extract() {
    cerr << endl;
    cerr << "Extracting lines from occupancy grid..." << endl;

    std::vector<cv::Vec2f> lines; //lines represented in polar coords (ro,theta)
    bool toPrint = true;
    ...
    for (int j = this->max_threshold; j > this->min_threshold; j--) {
        ...
        for (std::vector<cv::Vec2f>::iterator i = lines.begin(); i != lines.end(); i++) {
            float rho = (*i)[0], theta = (*i)[1];
            toPrint = true;

            for (std::vector<cv::Vec2f>::iterator it = this->extracted_lines.begin(); it != this->extracted_lines.end(); it++)
                if (std::fabs((*it)[0] - rho) < min_distance && this->isSameAngle((*it)[1], theta)) {
                    toPrint = false;
                    break;
                }
    ...
    cerr << "Extracted " << this->extracted_lines.size()-4 << " lines" << endl;
}

void LinesExtractor::computeMaxGridDistance() {
    cerr << endl;
    cerr << "Computing max grid distance..." << endl;

    for (std::vector<cv::Vec2f>::iterator i = this->extracted_lines.begin(); i != this->extracted_lines.end() - 1; ++i) {
        for (std::vector<cv::Vec2f>::iterator j = i + 1; j != this->extracted_lines.end(); ++j) {
            if (this->isSameAngle((*i)[1], (*j)[1])) {
                if (this->isSameAngle((*i)[1], 0.0f))
                    this->max_grid_spacing.width = std::min(
                            fabs((*i)[0] - (*j)[0]),
                        static_cast<float>(this->max_grid_spacing.width));
                else
                    this->max_grid_spacing.height = std::min(
                            fabs((*i)[0] - (*j)[0]),
                        static_cast<float>(this->max_grid_spacing.height));
            }
        }
    }

    cerr << "Width: " << this->max_grid_spacing.width << endl;
    cerr << "Height: " << this->max_grid_spacing.height << endl;
}

void LinesExtractor::makeGrid() {
    cerr << endl;
    cerr << "Making grid..." << endl;

    cv::Size image_size = this->image.size();
    std::queue<std::pair<cv::Vec2f, cv::Vec2f> > processing_queue;

    for (std::vector<cv::Vec2f>::iterator i = this->extracted_lines.begin(); i != this->extracted_lines.end(); i++) {
        ...
        for (std::vector<cv::Vec2f>::iterator j = this->extracted_lines.begin(); j != this->extracted_lines.end(); j++) {
            first_line = *i;
    ...
    while (!processing_queue.empty()) {
        ...
        float max_dist = (this->isSameAngle(in_process.first[1], 0.0)) ? max_grid_spacing.width : max_grid_spacing.height;

        if (std::floor(std::fabs(in_process.first[0] - in_process.second[0])/max_dist) >= 2.0) {
            cv::Vec2f new_line((in_process.first[0] + in_process.second[0])/2.0f, in_process.first[1]);
    ...
    cerr << "Added " << this->grid.size() << " lines" << endl;
}
...
LinesExtractor::LinesExtractor(const cv::Mat &image,
    ...
    cv::erode(this->dilated_image, this->dilated_image, cv::Mat(), cv::Point(-1, -1), 1);
