...
public:
    LinesExtractor(const cv::Mat &image,
                   const cv::Mat &original_image,
                   const int max_threshold=100,
                   const int min_threshold=90,
                   const float min_line_distance=5.0f,
                   const int active_pixel_range = 13,
                   const int active_pixel_threshold = 2);
    void processImage();
    ...
    const std::vector<cv::Vec2f>& getGrid(){return grid;}
