...
void SemanticMatrix::setDoor(
        const std::vector<cv::Point2i> &door_cells, Object &door) {
...
void SemanticMatrix::setObject(
        const std::vector<cv::Point2i> &object_cells, Object &object) {
...
void SemanticMatrix::setObjectTags(
        const std::vector<std::vector<cv::Point2i> > &cell_objects,
        std::vector<Object> &objects) {
    assert(cell_objects.size() == objects.size());

    std::vector<std::vector<cv::Point2i> >::const_iterator
            j = cell_objects.begin();
